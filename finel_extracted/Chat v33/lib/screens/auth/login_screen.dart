...
class _LoginScreenState extends State<LoginScreen>
  ...
  // ✅ FIX: roomId لو المستخدم جاي من JoinGateSheet
  String? get _pendingRoomId {
    try {
                    ...
                    child: Builder(
                      builder: (ctx) {
                        ...
                        return TabBarView(
                          controller: _tab,
                          children: [
                            _LoginTab(pendingRoomId: roomId),
                            _RegisterTab(pendingRoomId: roomId),
                            _GuestTab(pendingRoomId: roomId),
                          ],
                        );
                      },
                    ),
...
class _LoginTab extends StatefulWidget {
  ...
  const _LoginTab({this.pendingRoomId});
  @override
...
class _LoginTabState extends State<_LoginTab> {
  ...
  Future<void> _login() async {
    ...
    if (ok) {
      // ✅ FIX: لو في غرفة منتظرة، روح عليها مباشرة
      if (widget.pendingRoomId != null) {
        context.go('/room/${widget.pendingRoomId}');
      } else {
        context.go('/');
      }
    } else {
...
class _RegisterTab extends StatefulWidget {
  final String? pendingRoomId;
  const _RegisterTab({this.pendingRoomId});
  @override
...
class _RegisterTabState extends State<_RegisterTab> {
  ...
  Future<void> _register() async {
    ...
    setState(() => _loading = false);
    if (ok) {
      // ✅ FIX: بعد التسجيل روح للغرفة أو الهوم
      if (widget.pendingRoomId != null) {
        context.go('/room/${widget.pendingRoomId}');
      } else {
        context.go('/');
...
class _GuestTab extends StatefulWidget {
  final String? pendingRoomId;
  const _GuestTab({this.pendingRoomId});
  @override
...
class _GuestTabState extends State<_GuestTab> {
  ...
  Future<void> _enter() async {
    ...
    if (!mounted) return;
    // ✅ FIX: بعد دخول الزائر روح للغرفة مباشرة لو في غرفة منتظرة
    if (widget.pendingRoomId != null) {
      context.go('/room/${widget.pendingRoomId}');
    } else {
      context.go('/');
